...
namespace PxP
{
    ...
    {
        internal static double CurrentCutPosition = 0;  // 記錄目前裁切位置
        internal static int PieceLimit = 20;            // Piece 限制數量
        internal static int PieceTotal;                 // 記錄目前 Cut 幾片
        ...
        internal static int FreezPiece = 0;                      // 當 OnCut 或需要畫面凍結時紀錄當下跑到第幾片
        internal static int ImgRowsSet = 3;                      // ImgGrid 設定大小
        internal static int ImgColsSet = 3;
        internal static int PageSize = ImgRowsSet * ImgColsSet;  // 右下角 TableLayoutPanel 圖片數量
        internal static int PageCurrent = 0;
        internal static int PageTotal = 0;                       // 計算當OnCut發生時右下角 TableLayout 的頁數
        internal static string FlawGridViewOrderColumn = "";     // 右上角GridView排序的欄位
        internal static List<DoffGridColumns> DoffGridSetup = new List<DoffGridColumns>();  // 記錄右上角 DataGridView 欄位左右排序
